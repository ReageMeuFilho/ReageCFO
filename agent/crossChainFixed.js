...
const BASE_VAULT = "0x23742F2F911Ed434081177e5aA9DB6a5684ba0dC"; // NEW FIXED VAULT
...
async function main() {
    console.log("\n╔═══════════════════════════════════════════════════════════════╗");
    console.log("║  FIXED CROSS-CHAIN PAYMENT TEST - ReageCFO + LayerZero      ║");
    console.log("╚═══════════════════════════════════════════════════════════════╝\n");
    ...
    // Step 1: Temporarily transfer agent control
    ...
    // Payment details
    const recipient = "0x4444444444444444444444444444444444444444"; // Test recipient
    const amount = ethers.parseEther("0.001"); // 0.001 ETH
    const invoiceId = ethers.keccak256(ethers.toUtf8Bytes("FIXED-TEST-" + Date.now()));
    const intent = "Fixed cross-chain payment test";
    ...
    // Quote the fee
    console.log("💰 STEP 2: Quote LayerZero fee (with 500k gas)");
    ...
    console.log("🚀 STEP 3: Send cross-chain payment with FIXED payload");
    console.log("  Payload: abi.encode(address, uint256)");
    ...
    console.log("✅ Agent control restored\n");
    ...
    console.log("  Waiting 60 seconds for cross-chain delivery...");
    ...
    if (paymentCountAfter > paymentCountBefore) {
        ...
        console.log("  ✅ Payload mismatch FIXED!");
    } else {
        console.log("⏳ Message still in transit or failed");
    ...
    console.log("  ✅ EVVM: Sovereign ledger");
    console.log("  ✅ CDP: Agent wallet");
